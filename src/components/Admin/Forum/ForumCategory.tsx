...
export const ForumCategory: NextPage<IDashboardProps> = (props) => {
	...
	const deleteData = async (_id: string) => {
		try {
			const req = await fetch(`${SERVER_V1}/forum_category/${_id}`, {
				method: "DELETE",
				headers: {
			...
			const { success, message }: ForumCategoryQRes = await req.json();
	...
	const fillDataAll = async () => {
		try {
			...
			const { data, message }: ForumCategoryQRes = await req.json();
			if (req.status !== 200) {
	...
	const fillData = async (perPage: number, curPageQ: number) => {
		try {
			...
			const req = await fetch(SERVER_V1 + `/forum_category?perPage=${perPage}&page=${curPageQ}`, {
				method: "GET",
				headers: {
					"Content-Type": "application/json",
				},
			...
			const { data, message, page, pages }: ForumCategoryQRes = await req.json();
			if (req.status !== 200) {
